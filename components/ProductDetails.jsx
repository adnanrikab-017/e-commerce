...
export default function ProductDetails({ product }) {
  ...
  return <div className='flex max-lg:flex-col gap-12'>
    ...
    <div className='flex-1'>
      ...
      <div className='flex items-end gap-5 mt-10'>{cart[cartKey] && <div className='flex flex-col gap-3'><p className='text-lg text-slate-800 font-semibold'>Quantity</p><Counter productId={product.id} variantId={selectedVariantId || null} max={selectedVariant?.stock ?? product.stock} /></div>}<button disabled={soldOut} onClick={() => { if (product.variants?.length && !selectedVariantId) return toast.error('Select a size first'); if (!cart[cartKey]) dispatch(addToCart({ productId: product.id, variantId: selectedVariantId || null })); else router.push('/cart') }} className='bg-slate-800 text-white px-10 py-3 text-sm font-medium rounded hover:bg-slate-900 disabled:opacity-50'>{soldOut && selectedVariantId ? 'Sold Out' : !cart[cartKey] ? 'Add to Cart' : 'View Cart'}</button></div>
