...
export default function AddressModal({ setShowAddressModal, onSaved, initialAddress = null }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const data = await fetchJson('/api/addresses', { method: initialAddress ? 'PATCH' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(address) })
  ...
  return <div className='fixed inset-0 z-50 bg-white/60 backdrop-blur flex items-center justify-center p-6'>
    <form onSubmit={handleSubmit} className='relative flex flex-col gap-5 text-slate-700 w-full max-w-sm bg-white p-6 rounded-xl shadow-xl border border-slate-200'>
      ...
      <input name='name' minLength={2} onChange={(e) => setAddress({ ...address, name: e.target.value })} value={address.name} className='p-2 px-4 border border-slate-200 rounded' placeholder='Recipient name' required />
      <input name='phone' minLength={7} onChange={(e) => setAddress({ ...address, phone: e.target.value })} value={address.phone} className='p-2 px-4 border border-slate-200 rounded' placeholder='Phone' required />
      <textarea name='address' minLength={5} onChange={(e) => setAddress({ ...address, address: e.target.value })} value={address.address} className='p-2 px-4 border border-slate-200 rounded min-h-24' placeholder='Street address' required />
      <input name='area' minLength={2} onChange={(e) => setAddress({ ...address, area: e.target.value })} value={address.area} className='p-2 px-4 border border-slate-200 rounded' placeholder='Area / city' required />
